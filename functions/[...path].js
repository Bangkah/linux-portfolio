export async function onRequest(context) {
  ...
  // Variasi pesan jebakan berdasarkan apa yang mereka cari
  let customRoast = "";
  ...
  if (path.includes("admin") || path.includes("login") || path.includes("dashboard")) {
    ...
    customRoast = "[!] Mau nyari file backup? Isinya cuma folder node_modules 5GB sama meme Barcelona kalah El Clasico.";
...
`;
  ...
  return new Response(responseBody, {
    status: 200, // Sengaja 200 biar scanner mengira file itu benar-benar ada (false positive trap)
    headers: {
