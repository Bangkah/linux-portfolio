 ...
 * Shared logic for calling serverless functions
...
export const callServerlessFunction = async <T = unknown>(
  ...
  // Production: use the configured serverless API path
  const query = new URLSearchParams(params).toString();
  const url =
    query.length > 0
      ? `${SERVERLESS_API_PATH}${endpoint}?${query}`
      : `${SERVERLESS_API_PATH}${endpoint}`;
